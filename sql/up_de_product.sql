...
-- UPDATE FUNCTION
create
or replace function product_update_check()
    returns trigger
as
$$
declare
...
insert into project_2.public.product
values (max + 1, new.number, new.model, new.name, new.unit_price);
...
create trigger product_update_trigger
    ...
    for each row
    execute procedure product_update_check();
...
--DELETE  FUNCTION
create
or replace function product_delete_check()
    returns trigger
as
$$
begin
